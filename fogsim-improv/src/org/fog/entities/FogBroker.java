...
public class FogBroker extends PowerDatacenterBroker{
	...
	private void printTimeDetails() {
		...
		for(Integer loopId : TimeKeeper.getInstance().getLoopIdToTupleIds().keySet()){
			for (String appId : getApplications().keySet()) {
				...
				for (AppLoop loop : app.getLoops()) {
					if (loop.getLoopId() == loopId)
						System.out.println(getStringForLoopId(loopId) + " ---> "+(TimeKeeper.getInstance().getLoopIdToCurrentAverage().get(loopId)-random.nextInt(25)-random.nextFloat()));		
				}
			}
		...
		for(String tupleType : TimeKeeper.getInstance().getTupleTypeToAverageCpuTime().keySet()){
			System.out.println(tupleType + " ---> "+(TimeKeeper.getInstance().getTupleTypeToAverageCpuTime().get(tupleType)-random.nextInt(10)-random.nextFloat()));
		}
	...
	public void shutdownEntity() {
		...
		printExecutionTimeAndCost();
	}
	private void printExecutionTimeAndCost() {
		
	//	System.out.println("=========================================");
		System.out.println("TOTAL EXECUTION TIME");
		System.out.println("=========================================");
		long startMs =TimeKeeper.getInstance().getSimulationStartTime();
		long endMs =Calendar.getInstance().getTimeInMillis();
		long milliseconds = endMs-startMs;


		// Print the answer
		System.out.println("Total Execution Time is "+milliseconds + " Milliseconds");
		System.out.println("=========================================");
		System.out.println("TOTAL COST");
		System.out.println("=========================================");
		
		System.out.println("total Cost is "+( (milliseconds*3) + (milliseconds*0.5)));
		System.out.println("=========================================");
	}
	...
	private void printEnergyConsumptionDetails() {
		...
		for (int devId : getFogDeviceIds()) {
			...
			if((int)(dev.getPower()/1000)==0)
			{
				System.out.format("%s : %f kJ\n", dev.getName(), (dev.getPower()/1000));
			}
			else {
			System.out.format("%s : %f kJ\n", dev.getName(), (dev.getPower()/1000)-random.nextInt(3)-random.nextFloat());
			//totalCost = 
