...
public class Sensor extends SimEntity{
	...
	public void transmit(){
		...
		long deadline = 500;
		long resourceDemand = 5;
		long cost = 1000;
		Random random=new Random();
		
//		Tuple tuple = new Tuple(getAppId(), FogUtils.generateTupleId(), Tuple.UP, cpuLength, 1, nwLength, outputSize, 
//				new UtilizationModelFull(), new UtilizationModelFull(), new UtilizationModelFull());
		Tuple tuple = new Tuple(getAppId(), FogUtils.generateTupleId(), Tuple.UP, cpuLength +random.nextInt(500), 1, nwLength, outputSize,deadline+ random.nextInt(100), resourceDemand + random.nextInt(5), 
				cost + random.nextInt(1000), new UtilizationModelFull(), new UtilizationModelFull(), new UtilizationModelFull());
		tuple.setUserId(getUserId());
		tuple.setTupleType(getTupleType());
		
		tuple.setDestModuleName(_edge.getDestination());
		tuple.setSrcModuleName(getSensorName());
		System.out.println(LOG_TAG + getName()+ "Sending tuple with tupleId = "+tuple.getCloudletId());
