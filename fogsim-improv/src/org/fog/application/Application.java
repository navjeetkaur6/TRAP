...
//				mips, ram, bw, size, vmm, new TupleScheduler(mips, 1), new HashMap<Pair<String, String>, SelectivityModel>());
		...
		AppModule module = new AppModule(FogUtils.generateEntityId(), moduleName, appId, userId, 
				mips, ram, bw, size, vmm, noOfPes + random.nextInt(5),workLoad +random.nextInt(500),hopeCount + random.nextInt(50), new TupleScheduler(mips, 1), new HashMap<Pair<String, String>, SelectivityModel>());
		getModules().add(module);
	//	System.out.println("noOfPes "+module.getNoOfPes());
		List<AppModule> moduleList = getModules();
		if ( moduleList.size()>1)
		{
			List <AppModule> tempList = new ArrayList<AppModule>();
			List <AppModule> sortList = new ArrayList<AppModule>();
			for (AppModule virtual : moduleList)
			{
			...
			{
				AppModule smallestModule = tempList.get(0);
				for (AppModule checkModule :tempList)
				{
			...
			int count=1;
			for (AppModule modulePrint:sortList)
			{
				System.out.println(count +"  Name "+ modulePrint.getName()+ " PES No "+modulePrint.getNoOfPes()+ " WorkLoad "
						+modulePrint.getWorkLoad()+ " HopeCount " + modulePrint.getHopeCount());
				count++;
			}
		
			getModules().removeAll(moduleList);
			getModules().addAll(sortList);
			System.out.println("App Module created list "+getModules());
		}
	}
