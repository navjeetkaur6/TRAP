...
public class ModulePlacementOnlyCloud extends ModulePlacementPolicy {
	...
	public List<ModulePlacement> computeModulePlacements(
		...
		List<FogDeviceCharacteristics> sortedFogDeviceCharacteristics=sortFogDevices(fogDeviceCharacteristics);
		
		
		for (FogDeviceCharacteristics fc : sortedFogDeviceCharacteristics) {
...
//				break;
//		}
		if(sortedFogDeviceCharacteristics.get(0).isCloudDatacenter())
		{
			cloud=sortedFogDeviceCharacteristics.get(1);
		}
		else
		{
			cloud=sortedFogDeviceCharacteristics.get(0);
		}
		//cloud=getFogDeviceCharacteristics().get(getFogDeviceCharacteristics().keySet());
		if (cloud == null) {
	...
	public List<FogDeviceCharacteristics> sortFogDevices(List<FogDeviceCharacteristics> fogDeviceCharacteristics)
	{	
		List <FogDeviceCharacteristics> tempList = new ArrayList<FogDeviceCharacteristics>();
		List <FogDeviceCharacteristics> sortList = new ArrayList<FogDeviceCharacteristics>();
		for (FogDeviceCharacteristics virtual : fogDeviceCharacteristics)
		{
			tempList.add(virtual);
		}
		...
		{
			FogDeviceCharacteristics smallestModule = tempList.get(0);
			for (FogDeviceCharacteristics checkModule :tempList)
			{
				if(smallestModule.getNoOfPes() < checkModule.getNoOfPes())
				{
					smallestModule = checkModule;
				}
				else if (smallestModule.getNoOfPes() == checkModule.getNoOfPes())
				{
					if( smallestModule.getWorkLoad() > checkModule.getWorkLoad())
					{
						smallestModule = checkModule;
					}
					else if (smallestModule.getWorkLoad() == checkModule.getWorkLoad())
					{
						if(smallestModule.getHopeCount() > checkModule.getHopeCount())
						{
							smallestModule = checkModule;
						}
					}
			...
			sortList.add(smallestModule);
			tempList.remove(smallestModule);
		}
		System.out.println("sort list "+sortList);
		int count=1;
		for (FogDeviceCharacteristics modulePrint:sortList)
		{
			System.out.println(count +"  Name "+ modulePrint.getId()+ " PES No "+modulePrint.getNoOfPes()+ " WorkLoad "
					+modulePrint.getWorkLoad()+ " HopeCount " + modulePrint.getHopeCount());
			count++;
		}
	
		
		System.out.println("Fog created list "+ sortList.toString());
